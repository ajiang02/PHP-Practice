...
class Http
{
    ...
    protected function runWithRequest(Request $request)
    {
        ...
        // 设置开启事件机制：拿到 \config\app.php 下的 with_event 值
    ...
    {
        // 是否启用路由：拿到 \config\app.php 下的 with_route 值
        // 如果值为 false，则调用 loadRoutes()
        $withRoute = $this->app->config->get('app.with_route', true) ? function () {
            $this->loadRoutes();
        } : null;

        // 套路，获得 think\route 类实例
    ...
    protected function loadMiddleware(): void
    {
        if (is_file($this->app->getBasePath() . 'middleware.php')) {
            // 同样的套路，获得 think\middleware 类的实例,导入 app\middleware.php
